...
### Fixed
- Screen Recording permission dialog appeared on every launch even when permission was already granted. The system dialog is now shown only when the user explicitly triggers a capture, not at app startup.
- "Quit & Relaunch" button did not relaunch the app. Switched to `NSWorkspace.openApplication(at:configuration:)` which reliably starts a new instance.
- Separated the system permission request from the in-app permission alert so they no longer appear simultaneously, preventing an infinite permission loop.
